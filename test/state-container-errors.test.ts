...
import { setIn, updateIn, editIn, edit, deleteIn, setDevMode } from '../src/bedit.mjs'
...
describe('state container error handling', () => {
  ...
  it('should handle async state container operations with errors', async () => {
    const container = {
      get: () => ({ value: 1 }),
      set: () => { throw new Error('Async set error') }
    }
    const obj = { [$beditStateContainer]: container }
  ...
  it('should handle state container errors in deleteIn operations', () => {
    const container = {
      get: () => ({ required: 'value', optional: 'other' }),
      set: (newState: any) => {
        if (!('required' in newState)) {
          throw new Error('Required property cannot be deleted')
        }
      }
    }
    const obj = { [$beditStateContainer]: container }
