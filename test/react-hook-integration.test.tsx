...
import { $asyncPatchable } from '../src/symbols.mjs'
import { setDevMode } from '../src/patchfork.mjs'

setDevMode(true)

describe('React Hook Integration - Final Tests', () => {
  it('should successfully integrate usePatchforkState with React components', () => {
    function TestComponent() {
      // Test all major data types
      const [objectState, objectStore] = usePatchableState({
        count: 0,
        name: 'test',
      })
      const [arrayState, arrayStore] = usePatchableState([1, 2, 3])
      const [setStateState, setStateStore] = usePatchableState({
        tags: new Set(['a', 'b']),
      })
      const [mapState, mapStore] = usePatchableState({
      ...
      return (
        <div data-testid="integration-test">
          ...
          <div data-testid="object-section">
            ...
            <span data-testid="object-has-container">
              {String($asyncPatchable in objectStore)}
            </span>
          </div>
          ...
          <div data-testid="array-section">
            ...
            <span data-testid="array-has-container">
              {String($asyncPatchable in arrayStore)}
            </span>
          </div>
          ...
          <div data-testid="set-section">
            ...
            <span data-testid="set-has-a">
              {String(setStateState.tags.has('a'))}
            </span>
            <span data-testid="set-has-container">
              {String($asyncPatchable in setStateStore)}
            </span>
          </div>
          ...
          <div data-testid="map-section">
            ...
            <span data-testid="map-has-container">
              {String($asyncPatchable in mapStore)}
            </span>
          </div>
        </div>
      )
  ...
  it('should handle function initializers', () => {
    function TestComponent() {
      const [state1, state1Store] = usePatchableState(() => ({
        initialized: true,
        value: 42,
      }))
      ...
      return (
        <div data-testid="function-init-test">
          <span data-testid="func-initialized">
            {String(state1.initialized)}
          </span>
          <span data-testid="func-value">{state1.value}</span>
          <span data-testid="func-array">{state2.join('-')}</span>
          <span data-testid="func-has-container">
            {String($asyncPatchable in state1Store)}
          </span>
        </div>
      )
    }
    ...
    expect(screen.getByTestId('func-array')).toHaveTextContent(
      'func-init-array',
    )
  ...
  it('should properly reject primitive values', () => {
    function StringComponent() {
      const state = usePatchableState('invalid' as any)
      return <div>{state}</div>
    }

    function NumberComponent() {
      const state = usePatchableState(42 as any)
      return <div>{state}</div>
    }

    function BoolComponent() {
      const state = usePatchableState(true as any)
      return <div>{state}</div>
    }

    function NullComponent() {
      const state = usePatchableState(null as any)
      return <div>{state}</div>
    }

    expect(() => render(<StringComponent />)).toThrow(
      'usePatchableState can only be used with objects, arrays, maps, or sets.',
    )
    expect(() => render(<NumberComponent />)).toThrow(
      'usePatchableState can only be used with objects, arrays, maps, or sets.',
    )
    expect(() => render(<BoolComponent />)).toThrow(
      'usePatchableState can only be used with objects, arrays, maps, or sets.',
    )
    expect(() => render(<NullComponent />)).toThrow(
      'usePatchableState can only be used with objects, arrays, maps, or sets.',
    )
  })
    ...
    function TestComponent() {
      ...
      return (
        <div data-testid="nested-test">
          ...
          <span data-testid="has-work-tag">
            {String(state.data.tags.has('work'))}
          </span>
          <span data-testid="metadata-size">{state.data.metadata.size}</span>
          <span data-testid="version">
            {state.data.metadata.get('version')}
          </span>
          <span data-testid="has-container">
            {String($asyncPatchable in store)}
          </span>
        </div>
      )
    }
...
//
// The patchfork integration with the state container is tested separately in react.test.ts
