...
import { shallowMutateIn } from '../bedit.mjs'

describe('shallowMutateIn', () => {
  describe('Object mutations', () => {
    it('should perform shallow mutations on object properties', () => {
      const baseObj = {
        user: {
          name: 'John',
          age: 30,
      ...
      const result = shallowMutateIn(baseObj).user((user) => {
        // Can mutate top-level properties of user
        user.name = 'Jane'
        user.age = 25
        // Cannot mutate nested properties - they are readonly
        // user.profile.name = 'Jane' // This would be readonly
      })
  ...
  describe('Array mutations', () => {
    it('should perform shallow mutations on array elements', () => {
      const baseObj = {
        todos: [
      ...
      const result = shallowMutateIn(baseObj).todos((todos) => {
    ...
    it('should replace entire arrays', () => {
      ...
      const result = shallowMutateIn(baseObj).items((items) => {
    ...
    it('should handle nested arrays', () => {
      ...
      const result = shallowMutateIn(baseObj).categories[0].tasks((tasks) => {
        // Can mutate array methods
        tasks.length = 0
  ...
  describe('Mixed object and array mutations', () => {
    it('should handle complex nested structures', () => {
      ...
      const result = shallowMutateIn(baseObj).user.profile.preferences(
        (preferences) => {
          // Can replace entire objects
  ...
  describe('Function-based mutations', () => {
    it('should support direct mutations with access to current value', () => {
      ...
      const result = shallowMutateIn(baseObj).counter((counter) => {
    ...
    it('should support complex mutations', () => {
      ...
      const result = shallowMutateIn(baseObj).user.profile((profile) => {
        // Can mutate top-level properties
  ...
  describe('Edge cases', () => {
    it('should handle deep nested mutations', () => {
      ...
      const result = shallowMutateIn(baseObj).a.b.c.d.e.f.g.h.i.j((j) => {
        j.value = 'updated'
      })
  ...
  describe('Performance characteristics', () => {
    it('should perform shallow cloning (not deep cloning)', () => {
      ...
      const result = shallowMutateIn(baseObj).user.profile.settings(
        (settings) => {
